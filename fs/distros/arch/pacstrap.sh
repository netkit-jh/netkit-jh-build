...
EXTRA_PACKAGES=(base coreutils bash grep gawk file tar systemd sed)
...
configure_minimal_system() {
  ...
  mkdir -p "$DEST/dev"
  mkdir -m 0755 -p "$DEST"/var/{cache/pacman/pkg,lib/pacman} "$DEST"/run
  mkdir -m 0755 -p "$DEST"/var/log
  mkdir -m 1777 -p "$DEST"/tmp
  mkdir -m 0555 -p "$DEST"/{proc,dev}
  sed -ie 's/^root:.*$/root:$1$GT9AUpJe$oXANVIjIzcnmOpY07iaGi\/:14657::::::/' "$DEST/etc/shadow"
  touch "$DEST/etc/group"
  echo "netkit" > "$DEST/etc/hostname"
...
install_packages() {
  ...
  #LC_ALL=C /netkit-build/fs/distros/arch/arch-chroot "$DEST" /usr/bin/pacman \
